...
// Create Date:
// Design Name: matbi_wdma
// Module Name: matbi_wdma
// Project Name:
// Target Devices: Zybo Z7-20
// Tool Versions:
// Description: wdma hand rtl!!
// Dependencies:
//
// Revision:
...
output  [C_M_AXI_GMEM_WSTRB_WIDTH - 1:0] m_axi_gmem_WSTRB; // full use wdata.
...
input  [C_M_AXI_GMEM_ADDR_WIDTH-1:0] mem; // base address
...
localparam NUM_SAMPLE_IN_AXI_DATA = C_M_AXI_GMEM_DATA_WIDTH/8; // 64 bit
localparam AXI_DATA_SHIFT = $clog2(NUM_SAMPLE_IN_AXI_DATA); // 2^3 
...
reg [1:0] 	c_state_aw; // Current state  (F/F)
reg [1:0] 	n_state_aw; // Next state (Variable in Combinational Logic)

reg [1:0] 	c_state_w; // Current state  (F/F)
reg [1:0] 	n_state_w; // Next state (Variable in Combinational Logic)

reg [1:0] 	c_state_b; // Current state  (F/F)
reg [1:0] 	n_state_b; // Next state (Variable in Combinational Logic)
...
reg  [C_M_AXI_GMEM_ADDR_WIDTH-1:0] r_wdma_baseaddr; // base address
...
assign m_axi_gmem_AWSIZE		= 3'b011; // Burst Size : 8 Bytes. 2^3
...
always @(posedge ap_clk) begin
	if(ap_rst) begin
		...
		r_wdma_baseaddr				<= mem; // base address
    end
end
/////////////////////////////////////////// 1. main ctrl 
//make is_run Signals (1 tick)
always @(posedge ap_clk) begin
    if(ap_rst) begin
		tick_ff <= 0;
    end else begin
...
assign is_done = b_hs && is_b_last_hs; // wait last b hand shake
...
///////////////////////////////////////////// 3. Ctrl of AXI4 Write #3 channels
////////////////////////// AW
// ARADDR Cal

wire [C_M_AXI_GMEM_ADDR_WIDTH-NUM_SAMPLE_IN_AXI_DATA-1:0] remain_hs = r_num_total_stream_hs - r_hs_data_cnt;
...
// assign fifo_read_w = (w_s_idle_w | is_burst_done_w) & is_aw_req_pre ;
assign fifo_read_w = is_burst_done_w; // (chapter 20)
assign w_s_idle_w  = (c_state_w == S_IDLE);
assign w_s_run_w   = (c_state_w == S_RUN);
// assign is_burst_last_w = (r_burst_cnt_w+1 == r_burst_len_w);
assign is_burst_last_w = (r_burst_cnt_w+1 == AWLEN_w); // (chapter 20)
...
// always @(posedge ap_clk) begin
    // if(ap_rst) begin
		// r_burst_len_w <= 'b0;
    // end else if (fifo_read_w) begin
		// r_burst_len_w <= AWLEN_w;
    // end
// end
