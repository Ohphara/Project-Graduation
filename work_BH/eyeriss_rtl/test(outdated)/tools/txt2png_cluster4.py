...
def main():
    ...
    for direction in cls:
        txt=open("output_{}.txt".format(direction),'rb')
        ...
        pixels=np.reshape(pixels,(40,40,8,8))
        lena_out_part=np.zeros((320,320))
        for m in range(pixels.shape[0]):
            for n in range(pixels.shape[1]):
                for i in range(pixels.shape[2]):
                    for j in range(pixels.shape[3]):
                        lena_out_part[8*m+i][8*n+j]=int(pixels[m][n][i][j]/9.0)
    ...
    #             print(i,j)
    png_pixel = np.hstack((np.vstack((png_pixel[0], png_pixel[1])), np.vstack((png_pixel[2], png_pixel[3]))))
    # png_mf_python=cv2.imread(".\\lena_mf_python.png")
    # png_gray=cv2.imread(".\\lena_gray.png")

    # print(png_mf_python[:,:,0])
    cv2.imwrite(".\\out_reconstructed.png",png_pixel)
    # print(png_pixel)
    np.savetxt(".\\out_reconstructed.txt",png_pixel,fmt="%4d")
