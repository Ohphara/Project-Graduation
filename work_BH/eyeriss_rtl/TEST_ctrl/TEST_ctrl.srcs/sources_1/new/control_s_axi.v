...
//------------------------AXI write fsm------------------
assign axi_awready = (wstate == WRIDLE);
assign axi_wready  = (wstate == WRDATA);
assign axi_bresp   = 2'b00;  // assume OKAY
assign axi_bvalid  = (wstate == WRRESP);
assign wmask   = { {8{axi_wstrb[3]}}, {8{axi_wstrb[2]}}, {8{axi_wstrb[1]}}, {8{axi_wstrb[0]}} };
assign aw_hs   = axi_awvaild & axi_awready;
...
always @(posedge ACLK) begin
    if (ACLK_EN) begin
        if (aw_hs)
            waddr <= axi_awaddr[ADDR_BITS-1:0];
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_AP_CTRL && axi_wstrb[0] && axi_wdata[0])
            int_ap_start <= 1'b1;
        else if (ap_ready)
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_AP_CTRL && axi_wstrb[0])
            int_auto_restart <=  axi_wdata[7];
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_GIE && axi_wstrb[0])
            int_gie <= axi_wdata[0];
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_IER && axi_wstrb[0])
            int_ier <= axi_wdata[1:0];
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_RDMA_TRANSFER_BYTE_DATA_0)
            int_rdma_transfer_byte[31:0] <= (axi_wdata[31:0] & wmask) | (int_rdma_transfer_byte[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_RDMA_MEM_PTR_DATA_0)
            int_rdma_mem_ptr[31:0] <= (axi_wdata[31:0] & wmask) | (int_rdma_mem_ptr[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_WDMA_TRANSFER_BYTE_DATA_0)
            int_wdma_transfer_byte[31:0] <= (axi_wdata[31:0] & wmask) | (int_wdma_transfer_byte[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_WDMA_MEM_PTR_DATA_0)
            int_wdma_mem_ptr[31:0] <= (axi_wdata[31:0] & wmask) | (int_wdma_mem_ptr[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_AXI00_PTR0_DATA_0)
            int_axi00_ptr0[31:0] <= (axi_wdata[31:0] & wmask) | (int_axi00_ptr0[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_VALUE_TO_ADD)
            int_value_to_add[31:0] <= (axi_wdata[31:0] & wmask) | (int_value_to_add[31:0] & ~wmask);
    end
end
