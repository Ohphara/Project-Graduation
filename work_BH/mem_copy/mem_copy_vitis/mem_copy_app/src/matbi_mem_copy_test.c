...
//
// License : https://github.com/matbi86/matbi_verilogHDL_season_1/blob/master/LICENSE
// Create Date: 2023/05/03
// Design Name:
// Project Name:
// Target Devices:
// Tool Versions:
// Description: test matbi dma
//
// Dependencies:
//
// Revision:
...
void matbi_hw_memcpy(void* dest, const void* source, size_t num){
    u32 read_data;
	Xil_Out32((XPAR_MATBI_DMA_IP_TOP_0_BASEADDR) + ADDR_RDMA_TRANSFER_BYTE_DATA_0, (u32)num);
	Xil_Out32((XPAR_MATBI_DMA_IP_TOP_0_BASEADDR) + ADDR_RDMA_MEM_PTR_DATA_0, (u32) source );
	Xil_Out32((XPAR_MATBI_DMA_IP_TOP_0_BASEADDR) + ADDR_WDMA_TRANSFER_BYTE_DATA_0, (u32)num);
	Xil_Out32((XPAR_MATBI_DMA_IP_TOP_0_BASEADDR) + ADDR_WDMA_MEM_PTR_DATA_0, (u32) dest);

	while(1) {
		read_data = Xil_In32((XPAR_MATBI_DMA_IP_TOP_0_BASEADDR) + ADDR_AP_CTRL);
	    if( (read_data & CTRL_IDLE_MASK) == CTRL_IDLE_MASK ) // IDLE
	    	break;
	}
	Xil_Out32((XPAR_MATBI_DMA_IP_TOP_0_BASEADDR) + ADDR_AP_CTRL, (u32)(0x00000001)); // Start !!
...
//////////// HW Mem Copy //////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////
    	printf("======= Verilog Season2 matbi_mem_copy_test ======\n");
    	do{
    	...
    	Xil_Out32((XPAR_MATBI_DMA_IP_TOP_0_BASEADDR) + ADDR_AXI00_PTR0_DATA_0, (u32)(0x00000000)); // base addr no use now.
        Xil_Out32((XPAR_MATBI_DMA_IP_TOP_0_BASEADDR) + ADDR_ADD_TO_VALUE, (u32) add_val);

    	XTime_GetTime(&tStart);
    	matbi_hw_memcpy(wdma_baseaddr, rdma_baseaddr, transfer_cnt);
    	XTime_GetTime(&tEnd);
...
///////////////////////////////////////////////////////////////////////
    	...
    	XTime_GetTime(&tEnd);
		printf("SW Mem Copy function Time %.2f us.\n",
		       1.0 * (tEnd - tStart) / (COUNTS_PER_SECOND/1000000));
    	...
    	XTime_GetTime(&tEnd);
 		printf("SW no use dma Time %.2f us.\n",
		       1.0 * (tEnd - tStart) / (COUNTS_PER_SECOND/1000000));
    }
    return 0;
}
